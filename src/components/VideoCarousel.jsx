...
const VideoCarousel = () => {
	...
	useGSAP(() => {
		// slider animation to slide the video out of the screen & put in the next video
		gsap.to("#slider", {
			transform: `translateX(${-100 * videoId}%)`,
			duration: 2,
			ease: "power2.inOut", // shows visualizer
		});
	...
	useEffect(() => {
		...
		if (span[videoId]) {
			...
			let animation = gsap.to(span[videoId], {
				...
				// when the video has ended, changes the progress bar
				onComplete: () => {
					if (isPlaying) {
						gsap.to(videoDivRef.current[videoId], {
							width: "12px",
